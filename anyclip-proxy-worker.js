...
var CONSENSUS_URLS = [
  "https://49.13.145.234:9230/tor/status-vote/current/consensus-microdesc",
  "http://49.13.145.234:9230/tor/status-vote/current/consensus-microdesc"
];
/* Back-compat alias kept so anything else in the file referencing
 * CONSENSUS_URL keeps building. The new code uses CONSENSUS_URLS. */
var CONSENSUS_URL = CONSENSUS_URLS[1];
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/consensus" && request.method === "GET") {
      ...
      const fetched = await _fetchConsensusBytes(
        CONSENSUS_URLS, CONSENSUS_MAX_BYTES, 10e3
      );
      if (!fetched.ok) {
        const snap = {
          ts: Date.now(),
