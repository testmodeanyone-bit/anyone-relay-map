...
/* === BEGIN inlined anyclip-prompts.js (paste at proxy top, right after the
 *     const _kvSchema = (function(){...})(); block, ~line 46+) ===
 * Canonical source: anyclip-prompts.js. If you change one, change both. */
const _anyclipPrompts = (function () {
/* ============================================================================
...
const ANYCLIP_MAX_TOKENS = { assistant: 600, moderate_pin: 150, moderate_report: 150 };
...
/* Optional per-conversation memory / dynamic context (operator info, relay
 * lookup, conversation memory). UNTRUSTED data — fenced and capped exactly like
 * stats so it can't smuggle instructions. Returns '' when empty. */
function _memoryBlock(raw) {
  if (typeof raw !== 'string') return '';
  let s = raw.replace(/[\u27E6\u27E7`]/g, '').trim();
  if (!s) return '';
  if (s.length > 2000) s = s.slice(0, 2000) + '\u2026';
  return `${_U_OPEN}
=== DYNAMIC CONTEXT (memory / operator / relay lookup) ===
${s}
${_U_CLOSE}`;
}
...
Respond with EXACTLY one JSON object on a single line, nothing else:
{"action": "dismiss"} or {"action": "mute", "reason": "<brief>"} or {"action": "block", "reason": "<brief>"}
No prose, no code fences.`;
...
function buildSystemPrompt(task, opts) {
  ...
  if (task === 'assistant') {
    ...
    const mem = _memoryBlock(opts.memory);
    if (mem) sys += '\n\n' + mem;
...
function parseModerationVerdict(task, modelText) {
  const fallback = task === 'moderate_pin'
    ...
    : { action: 'dismiss', reason: 'moderation unavailable' };
  ...
  // moderate_report
  const action = (p.action === 'block' || p.action === 'mute' || p.action === 'dismiss') ? p.action : 'dismiss';
  return { action, reason: typeof p.reason === 'string' ? p.reason.slice(0, 120) : '' };
}
  return {
    ANYCLIP_PROMPTS_VERSION, ANYCLIP_MODEL, ANYCLIP_MAX_TOKENS,
    STAT_FIELDS, sanitizeStats, buildSystemPrompt, parseModerationVerdict
  };
})();
/* === END inlined anyclip-prompts.js === */
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      try {
        ...
        /* ---- messages: untrusted data, strictly shaped ---- */
        ...
        for (const m of messages) {
          if (!m || typeof m !== 'object' ||
            ...
            /* Reject anything that isn't a plain user/assistant text turn.
             * In particular a client-sent role:"system" lands here and is
             * refused — the system layer is ours, not the client's. */
            return cors(JSON.stringify({ error: { message: "Invalid message shape" } }), 400);
          }
          totalChars += m.content.length;
        }
        if (totalChars > 24000) {
          return cors(JSON.stringify({ error: { message: "Conversation too long" } }), 400);
        }

        /* ---- task selection: server owns the prompt + model ---- */
        const task = (typeof body.task === 'string' && body.task) ? body.task : 'assistant';
        let built;
        try {
          built = _anyclipPrompts.buildSystemPrompt(task, {
            stats:  body.stats,            // sanitized inside the builder
        ...
        const requestedMax = Number.isFinite(body.max_tokens) ? body.max_tokens : built.maxTokens;
        const maxTokens = Math.min(Math.max(1, requestedMax), built.maxTokens);
        ...
        const anthropicRes = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            model: built.model,          // server-pinned, NOT body.model
            max_tokens: maxTokens,
            system: built.system,        // server-owned, NOT body.system
            messages: messages
          })
        });
        ...
        /* ---- moderation tasks: fail-closed structured verdict ---- */
        if (task === 'moderate_pin' || task === 'moderate_report') {
          const modText = (anthropicData && anthropicData.content && anthropicData.content[0] && anthropicData.content[0].text) || '';
          const verdict = _anyclipPrompts.parseModerationVerdict(task, modText);
          /* Always 200 with the verdict; the trusted caller acts on it. A model
           * or upstream failure already collapsed to the SAFE default inside
           * parseModerationVerdict (deny pin / action:none). */
          return cors(JSON.stringify(verdict), 200);
        }

        /* ---- assistant: pass the model response through unchanged ---- */
        return cors(JSON.stringify(anthropicData || { error: { message: "Upstream error" } }), anthropicRes.status);
      } catch (err) {
